...
private:
  ...
  // TODO GUI BORROW SECRET need this?
  void packet_borrow_next_secret(const call_t &call) {
    assert(call.extra_vars.count("the_chunk") &&
           "packet_borrow_next_secret without \"the_chunk\" extra var");
    assert(!call.extra_vars.at("the_chunk").second.isNull() &&
           "packet_borrow_next_secret with invalid \"the_chunk\" expression");

    assert(call.args.find("length") != call.args.end() &&
           "packet_borrow_next_secret without \"length\" variable");
    assert(!call.args.at("length").expr.isNull() &&
           "packet_borrow_next_secret with invalid \"length\" expression");
...
public:
  ...
  PacketManager(std::shared_ptr<KleeInterface> _klee_interface,
    ...
    call_handler_map["packet_borrow_next_secret"] =
        &PacketManager::packet_borrow_next_secret;
