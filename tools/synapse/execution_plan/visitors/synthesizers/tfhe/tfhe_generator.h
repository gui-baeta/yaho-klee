...
public:
    tfheGenerator()
        : Synthesizer(GET_BOILERPLATE_PATH(BOILERPLATE_FILE)),
          ...
          transpiler(*this),
          pending_ifs(nf_process_builder) {}

    std::string transpile(klee::ref<klee::Expr> expr);
    virtual void generate(ExecutionPlan &target_ep) override {
        visit(target_ep);
    }
    ...
    void visit(const ExecutionPlanNode *ep_node,
               const target::PacketBorrowNextSecret *node) override;
    void visit(const ExecutionPlanNode *ep_node,
               const target::TruthTablePBS *node) override;
    void visit(const ExecutionPlanNode *ep_node,
